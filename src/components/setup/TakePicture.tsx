...
export const TakePicture: React.FC<TakePictureProps> = ({
  ...
  return (
    <div className="flex flex-col gap-4 pb-8">
      <h2 className="pb-0 text-center text-2xl font-[700] text-gray-800 md:text-3xl">
        Let&apos;s bring your child&apos;s favorite toy to life!
      </h2>
      <p className="max-w-[600px] pb-4 text-center text-base text-gray-600">
      ...
      <div className="mx-auto mt-3 mb-12">
        <Card className="flex w-80 rotate-2 flex-col items-center gap-3 rounded-xs sm:w-96">
          {capturedImage ? (
            <Image
              src={capturedImage}
              alt="Captured toy"
              width={300}
              height={300}
              className="h-64 w-64 rounded-md object-cover sm:h-75 sm:w-75"
            />
          ) : (
            <div className="h-64 w-64 rounded-md border border-gray-300 bg-gray-100 sm:h-75 sm:w-75"></div>
          )}
