 ...
 */
export async function saveToyDataToFile(toy: ToyData): Promise<void> {
...
export async function saveUserDataToFile(data: UserData): Promise<void> {
  try {
    // Save to public/toy-photos directory
    let uploadDir = '';
    if (process.cwd() === '/var/task') {
      ...
      try {
        ...
        if (existingData && existingData.toys && Array.isArray(existingData.toys)) {
          ...
          const existingToysMap = new Map(
            (existingData.toys as ToyData[]).map(
              (toy: ToyData) => [toy.key, toy] as [string, ToyData]
            )
          );
          ...
          console.log(
            `Merged ${newToys.length} toys with ${existingData.toys.length} existing toys`
          );
        }
      } catch (readError) {
